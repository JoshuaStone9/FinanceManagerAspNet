    ...
    IReserveRecommendationService recommendationService) : IDashboardSummaryService
{
        ...
        IReadOnlyList<ReservePot> pots)
    {
        ...
        var dueReminders = await repo.GetFinanceRemindersAsync(
            "Open",
            dueOnly: true);
        ...
        var upcomingTargets = activePots
            ...
            .Select(x => new DashboardUpcomingTarget(
                ...
                Math.Max(
                    0m,
                    x.TargetAmount!.Value - x.AllocatedAmount)))
            .ToList();
        ...
        var completedCount = activePots.Count(x =>
            x.TargetAmount.HasValue &&
            x.AllocatedAmount >= x.TargetAmount.Value);
        ...
        var partiallyFundedCount = currentSummaries.Count(x =>
            x.CurrentStatus == "Partially funded");
        ...
        return new DashboardIntelligenceSummary
        {
            ...
            HealthyCount = Math.Max(
                0,
                activePots.Count - unhealthyIds.Count),

            ApproachingTargetCount = upcomingTargets.Count(x =>
                x.DaysRemaining <= 30),
