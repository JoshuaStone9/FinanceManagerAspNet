...
{
    public async Task<IActionResult> Index(int? potId, string? eventType, DateTime? from, DateTime? to)
    {
        return View(new FinanceEventsViewModel
        {
            ...
            Events = await repo.GetFinanceEventsAsync(potId, eventType, from, to)
        });
    }
}
